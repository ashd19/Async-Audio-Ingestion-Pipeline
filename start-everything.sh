...
echo $! > ../api.pid
cd ..
echo -e "${GREEN}✅ API is starting in the background (Logs: api.log)${NC}"
...
while ! curl -s http://localhost:8080/api/jobs/1 > /dev/null 2>&1; do
    # We check /api/jobs/1 because it returns 404/200 if up, or connection refused if down
    # But wait, we need to distinguish between "server down" and "job not found"
    if [ $COUNT -ge $MAX_RETRIES ]; then
        echo -e "${RED}❌ API took too long to start. Check api.log for errors.${NC}"
        exit 1
    fi
    sleep 2
    COUNT=$((COUNT+1))
...
echo -e "API Logs:         ${BLUE}tail -f api.log${NC}"
echo -e "Worker Logs:      ${BLUE}tail -f workers/*.log${NC}"
echo "--------------------------------------------"
echo -e "To stop everything, run: ${YELLOW}./stop-everything.sh${NC}"
